...
frappe.ui.form.on('The Subcontracts', {
    ...
    on_submit: async function (frm) {
        try {
            ...
            if (frm.doc.docstatus !== 1) {
                frappe.msgprint("السابكونتراكت غير معتمد بعد. لا يمكن تنفيذ العمليات.");
                return; // لا نقوم بأي عملية إذا لم يكن السابكونتراكت معتمدًا
            }
...
async function update_contract_values(contract_name) {
    try {
        ...
        for (const subcontract of subcontracts.message) {
            ...
            // التحقق من حالة المستند
            if (subcontract_doc.docstatus !== 1) {
                frappe.msgprint(`السابكونتراكت (${subcontract.name}) غير معتمد بعد. لا يمكن تحديث العقد الرئيسي.`);
                return; // لا نقوم بأي عملية إذا لم يكن السابكونتراكت معتمدًا
            }
